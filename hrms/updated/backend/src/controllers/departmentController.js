const { Department, Company, Employee } = require('../models'); // Import Employee for getDepartmentEmployees
  ...
  static async getDepartmentHierarchy(req, res, next) {
    try {
      const departments = await Department.model.findAll({
        attributes: ['id', 'name', 'parent_department_id'],
        order: [['name', 'ASC']],
      });
  ...
  static async getDepartmentEmployees(req, res, next) {
    try {
      ...
      const employees = await Employee.model.findAll({
        ...
        attributes: ['id', 'first_name', 'last_name', 'designation_id'],
        include: [
          {
            model: Department.model,
            as: 'department',
